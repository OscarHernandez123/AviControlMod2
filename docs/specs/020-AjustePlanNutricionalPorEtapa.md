...
3. **Scenario**: Ajuste de días de etapa por contingencia de galpón (desplazamiento en cascada)
   ...
   - **When** el nutricionista extiende la duración de la etapa "Inicio" hasta el día 26 debido a una contingencia (ej. bajo peso o cuarentena sanitaria)
   - **Then** el sistema actualiza la etapa "Inicio" para abarcar los días 8 al 26 manteniendo la ración configurada para dicha etapa
   - **And** desplaza automáticamente el inicio de la etapa "Engorde" al día 27 sin solapamientos ni días vacíos, recalculando la demanda diaria proyectada.
...
- **Ajuste y prórroga de días de etapa por galpón (contingencia sanitaria o bajo peso)**: Cuando un galpón requiera extender o reducir los días de una etapa nutricional (ej. por cuarentena sanitaria o retraso en curva de peso), el sistema debe permitir personalizar el rango de días de la etapa para ese galpón y desplazar automáticamente en cascada las etapas posteriores (`DiaInicio = DiaFinEtapaAnterior + 1`). El cálculo de ración y demanda diaria se mantendrá acorde a la etapa activa prorrogada.
- **Inmutabilidad de etapas transcurridas en el galpón**: Al personalizar la duración de etapas de un galpón, el sistema debe bloquear la modificación de etapas cuyo rango de días sea anterior a la edad actual de las aves del lote.
- **Validación temporal de la etapa activa**: El día de finalización de una etapa activa no puede fijarse en un día menor a la edad actual de vida del lote.
- **Extensión que sobrepasa la fecha proyectada de sacrificio**: Si una etapa previa se extiende más allá de la fecha estimada de sacrificio del lote, el sistema debe emitir una advertencia para actualizar la fecha proyectada de salida/sacrificio.
...
- **FR-010**: El sistema DEBE permitir personalizar el rango de días de duración de las etapas de alimentación para un galpón específico ante contingencias (cuarentena sanitaria, bajo peso o criterio veterinario), requiriendo el registro de una justificación/motivo y usuario responsable.
- **FR-011**: Al modificar el día final de una etapa en un galpón, el sistema DEBE aplicar un desplazamiento automático en cascada sobre las etapas posteriores (`DiaInicio = DiaFinEtapaAnterior + 1`), garantizando continuidad sin solapamientos de días.
- **FR-012**: El cálculo de demanda diaria y requerimiento de alimento de un galpón DEBE regirse estrictamente por la etapa y ración vigentes según el calendario ajustado del galpón y su edad actual de vida.
- **FR-013**: El sistema DEBE restringir la edición de etapas transcurridas en el galpón y validar que la fecha de fin de la etapa activa sea mayor o igual al día actual de vida del lote.
...
- **Galpon**: Representa el galpón con su población y estado de ciclo.
  - *Atributos*: ID, nombre, capacidad, poblacionAvesActivas, etapaActual, edadDias.
- **CalendarioAlimentacionGalpon**: Calendario de duración de etapas personalizado por galpón para gestionar excepciones y prórrogas.
  - *Atributos*: ID, galponId, etapa, diaInicio, diaFin, justificacionAjuste, fechaAjuste.
...
- **SC-005**: 100% de los desplazamientos de etapas en galpones con excepciones sanitarias o de peso se recalculan en cascada sin solapamientos ni días desprovistos de ración.
