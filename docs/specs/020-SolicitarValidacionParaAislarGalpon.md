...
1. **Scenario**: Bloqueo de solicitud duplicada en trámite
   - **Given** que un galpón en estado "En producción" ya tiene una solicitud previa en estado "Pendiente de validación"
...
- **Galpón**: Representa la unidad física de producción avícola en producción para la cual se solicita la revisión.
  - Atributos utilizados: `UUID único`, `Nombre`, `Aforo máximo`, `Estado` (`En producción`).
...
- **SC-002**: El 100 % de las solicitudes creadas conserva el estado `En producción` del galpón sin alterarlo hasta la resolución veterinaria.
- **SC-003**: El 100 % de los intentos de crear solicitudes sobre galpones en estados distintos a `En producción` es rechazado por el sistema.
