...
Como administrador, quiero programar la fecha y hora del sacrificio de un lote declarado apto para que el sistema gestione la orden y libere el galpón en el momento programado.
...
**Independent Test**: Se puede probar validando un lote como `Apto`, programando una fecha y hora futuras y verificando que la orden pueda reprogramarse o cancelarse antes de su ejecución. Al llegar el momento programado, el sistema debe cambiar el galpón a `Vacío` y finalizar el alojamiento del lote.

**Acceptance Scenarios**:

1. **Scenario**: Programación correcta del sacrificio
   - **Given** que el administrador obtuvo el resultado `Apto` para el lote dentro del mismo flujo
   - **When** programa el sacrificio para una fecha y hora futuras
   - **Then** el sistema crea la orden asociada al lote y al galpón, y mantiene el galpón en estado `En producción` hasta el momento programado
...
2. **Scenario**: Ejecución de la orden en la fecha programada
   ...
   - **Then** el sistema marca la orden como ejecutada, cambia el estado del galpón a `Vacío` y finaliza el alojamiento del lote

3. **Scenario**: Intento de programación sin una validación apta
   - **Given** que el administrador no ha obtenido el resultado `Apto` dentro del mismo flujo
...
4. **Scenario**: Intento de programación con una fecha u hora no futura
   - **Given** que el administrador obtuvo el resultado `Apto`
...
5. **Scenario**: Reprogramación antes de la ejecución
   ...
   - **Then** el sistema actualiza la programación y conserva el galpón en estado `En producción`
...
- **Edge case #1 - Cambio del lote o del estado del galpón antes de confirmar la orden**
  ...
  - ¿Cómo maneja el sistema un galpón cuyo lote alojado o estado cambia después de obtener el resultado `Apto` y antes de confirmar la programación?  
    El sistema debe comprobar que el mismo lote continúa alojado y que el galpón permanece en estado `En producción`. Si alguno de estos datos cambió, debe rechazar la programación e informar la inconsistencia.
...
- **Edge case #3 - El sistema no está disponible en el momento programado**
  ...
  - ¿Cómo maneja el sistema una orden cuya fecha y hora se cumplen mientras el sistema se encuentra temporalmente fuera de servicio?  
    Al restablecerse, el sistema debe identificar la orden vencida, ejecutarla una sola vez, cambiar el galpón a `Vacío` y finalizar el alojamiento del lote.
...
- **FR-002**: La opción de programar el sacrificio DEBE estar disponible únicamente después de obtener el resultado `Apto` para el lote dentro del mismo flujo de validación.
...
- **FR-006**: El galpón DEBE permanecer en estado `En producción` mientras la orden esté programada o sea reprogramada, y la cancelación NO DEBE modificar el galpón ni el alojamiento del lote.
- **FR-007**: Al llegar la fecha y hora programadas, el sistema DEBE marcar la orden como ejecutada, cambiar el galpón a `Vacío` y finalizar el alojamiento del lote.
...
- **SC-001**: Al menos el 90 % de los administradores puede programar el sacrificio en menos de 30 segundos después de obtener el resultado `Apto`.
...
- **SC-003**: Al menos el 99 % de las órdenes ejecutadas cambia el galpón a `Vacío` dentro del primer minuto posterior a la fecha y hora programadas.
